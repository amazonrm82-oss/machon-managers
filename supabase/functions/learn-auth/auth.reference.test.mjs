...
// Every account, the administrator included, must be name@icelp.org.il.
const ADMIN='matanz@icelp.org.il';
function emailAllowed(e){ return /^[^@\s]+@icelp\.org\.il$/.test(String(e||'').trim().toLowerCase()); }
...
// --- email domain rule (everyone must be name@icelp.org.il) ---
chk('icelp domain allowed', emailAllowed('worker@icelp.org.il'));
chk('admin also uses icelp domain', emailAllowed('Matanz@icelp.org.il'));
chk('other domain rejected', !emailAllowed('someone@gmail.com'));
chk('lookalike ice.org.il rejected', !emailAllowed('bob@ice.org.il'));
